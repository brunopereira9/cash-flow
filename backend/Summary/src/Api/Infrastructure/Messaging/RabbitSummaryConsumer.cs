...
public sealed class RabbitSummaryConsumer(
    ...
    ILogger<RabbitSummaryConsumer> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogWarning(exception, "Summary consumer stopped; retrying.");
    ...
    private void Consume(CancellationToken token)
    {
        ...
        {
            var parent = Propagators.DefaultTextMapPropagator.Extract(default, delivery.BasicProperties?.Headers,
                static (headers, key) => headers is not null && headers.TryGetValue(key, out var value)
                    ?
                    [
                        Encoding.UTF8.GetString(value is byte[] bytes
                            ? bytes
                            : Encoding.UTF8.GetBytes(value.ToString() ?? string.Empty))
                    ]
                    : []);
            using var activity = CashFlowSummaryTelemetry.Messaging.StartActivity("cashflow.summary consume",
                System.Diagnostics.ActivityKind.Consumer, parent.ActivityContext);
            ...
            try
            {
                ...
                logger.LogInformation("Business event {BusinessEvent} processed with {MessageId}",
                    "summary.projection.applied", delivery.BasicProperties?.MessageId);
                activeChannel.BasicAck(delivery.DeliveryTag, false);
            }
            catch (Exception exception)
            {
                activity?.SetStatus(System.Diagnostics.ActivityStatusCode.Error, exception.Message);
                logger.LogError(exception, "Ledger event failed");
