...
public sealed class SummaryRepository(SummaryDbContext db) : ISummaryRepository
{
    ...
    public async Task<bool> ApplyProjectionAsync(ProjectionEvent message, CancellationToken token)
    {
        ...
        {
            CashFlowSummaryTelemetry.InboxDuplicates.Add(1);
            return false;
        }
        ...
        if (current is not null && current.Version >= message.Version)
        {
        ...
        {
            affected.Add(current.BusinessDate);
            current.Apply(message.Entry);
        }
        ...
        await db.SaveChangesAsync(token);
        await transaction.CommitAsync(token);
        CashFlowSummaryTelemetry.ProjectionApplied.Add(1);
        return true;
    }
