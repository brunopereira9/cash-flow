...
public class DailySummaryFreshnessTests : IClassFixture<RabbitMqFixture>
{
    ...
    public async Task ReflectsConfirmedLedgerMutationWithinThirtySeconds()
    {
        ...
        await TestEventually.WaitForAsync(
            async () => await HasSummaryAsync(summaryClient, firstDate, credits: 0m, debits: 10m, balance: -10m),
        ...
        await TestEventually.WaitForAsync(async () =>
                ...
                await HasSummaryAsync(summaryClient, movedDate, credits: 30m, debits: 0m, balance: 30m),
            TimeSpan.FromSeconds(30));
        ...
        await TestEventually.WaitForAsync(
            async () => await HasSummaryAsync(summaryClient, movedDate, credits: 0m, debits: 0m, balance: 0m),
            TimeSpan.FromSeconds(30));
    ...
    private static async Task<bool> HasSummaryAsync(HttpClient client, DateOnly date, decimal credits, decimal debits,
        decimal balance)
    {
