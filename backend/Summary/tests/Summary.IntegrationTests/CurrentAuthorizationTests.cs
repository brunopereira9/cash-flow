...
internal sealed class SummaryKeycloakProtocolHandler(Func<ProtocolSummaryKeycloakState> state) : HttpMessageHandler
{
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (request.Method == HttpMethod.Post && path.EndsWith("/protocol/openid-connect/token", StringComparison.Ordinal))
        {
            ...
            return Response(HttpStatusCode.OK, "{\"access_token\":\"protocol-admin-token\",\"token_type\":\"Bearer\",\"expires_in\":300}");
        }
        ...
        if (request.Method == HttpMethod.Get && path.EndsWith("/role-mappings/realm", StringComparison.Ordinal))
        {
            ...
            return Response(HttpStatusCode.OK, $"[{{\"id\":\"operator-role\",\"name\":\"{current.Role}\",\"containerId\":\"cashflow\"}}]");
        }

        if (request.Method == HttpMethod.Get && path.Contains("/admin/realms/cashflow/users/", StringComparison.Ordinal))
        {
            if (!HasAdminBearer(request)) return Response(HttpStatusCode.Unauthorized, "{}");
            return Response(HttpStatusCode.OK, $"{{\"id\":\"demo-operator\",\"username\":\"demo-operator\",\"enabled\":{current.Enabled.ToString().ToLowerInvariant()}}}");
        }

        return Response(HttpStatusCode.NotFound, "{}");
    }

    private static bool HasAdminBearer(HttpRequestMessage request) => request.Headers.Authorization?.Scheme == "Bearer" && request.Headers.Authorization.Parameter == "protocol-admin-token";
...
public sealed class SecuredSummaryApiFactory : MigratedSummaryApiFactory
{
    ...
    private readonly string database = Path.Combine(Path.GetTempPath(), $"cashflow-secured-summary-{Guid.NewGuid():N}.db");
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.UseSetting("Database:Provider", "Sqlite"); builder.UseSetting("ConnectionStrings:Summary", $"Data Source={database}"); builder.UseSetting("RabbitMq:Enabled", "false");
        builder.UseSetting("Keycloak:Enabled", "true"); builder.UseSetting("Keycloak:Authority", Issuer); builder.UseSetting("Keycloak:Audience", Audience); builder.UseSetting("Keycloak:ValidationSigningKey", SigningKey);
        builder.UseSetting("Keycloak:AdminClientId", "cashflow-api"); builder.UseSetting("Keycloak:AdminClientSecret", "test-secret");
        builder.ConfigureTestServices(services =>
        {
            services.AddHttpClient("keycloak-current-state").ConfigurePrimaryHttpMessageHandler(() => new SummaryKeycloakProtocolHandler(() => State));
...
public class SummaryCurrentAuthorizationTests : IClassFixture<SecuredSummaryApiFactory>
{
    ...
    public async Task AppliesCurrentAccountStateAndRoleToTheNextSummaryRequest()
    {
        ...
        if (!initial.IsSuccessStatusCode) Assert.Fail($"{initial.StatusCode}: {await initial.Content.ReadAsStringAsync()}");
    ...
    private static string Token() => new JwtSecurityTokenHandler().WriteToken(new JwtSecurityToken(issuer: SecuredSummaryApiFactory.Issuer, audience: SecuredSummaryApiFactory.Audience, claims: [new Claim("sub", "demo-operator")], notBefore: DateTime.UtcNow.AddMinutes(-1), expires: DateTime.UtcNow.AddMinutes(5), signingCredentials: new SigningCredentials(new SymmetricSecurityKey(Encoding.UTF8.GetBytes(SecuredSummaryApiFactory.SigningKey)), SecurityAlgorithms.HmacSha256)));
}