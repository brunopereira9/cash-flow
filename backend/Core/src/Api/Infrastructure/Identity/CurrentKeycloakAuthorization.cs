...
public sealed class CurrentKeycloakAuthorization(IHttpClientFactory clients, IConfiguration configuration) : ICurrentKeycloakAuthorization
{
    public async Task<CurrentKeycloakState> ConfirmAsync(ClaimsPrincipal principal, CancellationToken requestToken)
    {
        ...
        using var userRequest = new HttpRequestMessage(HttpMethod.Get, $"{baseUri}/admin/realms/{realm}/users/{Uri.EscapeDataString(subject)}");
        ...
        var enabled = userDocument.RootElement.TryGetProperty("enabled", out var enabledValue) && enabledValue.GetBoolean();
        using var rolesRequest = new HttpRequestMessage(HttpMethod.Get, $"{baseUri}/admin/realms/{realm}/users/{Uri.EscapeDataString(subject)}/role-mappings/realm");
        ...
        using var rolesDocument = JsonDocument.Parse(await rolesResponse.Content.ReadAsStringAsync(timeoutSource.Token));
        var roles = rolesDocument.RootElement.EnumerateArray().Select(role => role.GetProperty("name").GetString()!).ToHashSet(StringComparer.Ordinal);
    ...
    private async Task<string> AdminTokenAsync(HttpClient client, string authority, CancellationToken token)
    {
        ...
        var clientSecret = configuration["Keycloak:AdminClientSecret"] ?? throw new InvalidOperationException("Keycloak admin client secret is required.");
...
public static class CurrentKeycloakAuthorizationApplicationBuilderExtensions
{
    public static IApplicationBuilder UseCurrentKeycloakAuthorization(this IApplicationBuilder app, IConfiguration configuration)
    {
        ...
        {
            if (!context.Request.Path.StartsWithSegments("/ledger") && !context.Request.Path.StartsWithSegments("/audit")) { await next(); return; }
            if (context.User.Identity?.IsAuthenticated != true) { await context.ChallengeAsync(); return; }
            try
            {
                var state = await context.RequestServices.GetRequiredService<ICurrentKeycloakAuthorization>().ConfirmAsync(context.User, context.RequestAborted);
                if (!state.Enabled || !HasBusinessRole(state.Roles) || (IsMutation(context.Request) && !HasWriteRole(state.Roles))) { context.Response.StatusCode = StatusCodes.Status403Forbidden; return; }
                await next();
            }
            ...
            { CashFlowCoreTelemetry.KeycloakFailures.Add(1); context.Response.StatusCode = StatusCodes.Status503ServiceUnavailable; }
        });
    }

    private static bool HasBusinessRole(IReadOnlySet<string> roles) => roles.Count(role => role is "admin" or "operator" or "auditor") == 1;
    private static bool HasWriteRole(IReadOnlySet<string> roles) => roles.Contains("admin") || roles.Contains("operator");
