...
public sealed class CoreSwaggerExamplesOperationFilter : IOperationFilter
{
    public void Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        if (path == "ledger/entries" && context.ApiDescription.HttpMethod == "POST" && operation.RequestBody?.Content.TryGetValue("application/json", out var requestContent) == true)
        ...
        if (path == "ledger/entries" && context.ApiDescription.HttpMethod == "GET" && operation.Responses.TryGetValue("200", out var response) && response.Content.TryGetValue("application/json", out var responseContent))
        ...
        if (path == "healthz" && operation.Responses.TryGetValue("200", out var healthResponse) && healthResponse.Content.TryGetValue("application/json", out var healthContent))
            healthContent.Examples["default"] = new OpenApiExample { Value = new OpenApiObject { ["status"] = new OpenApiString("ok"), ["service"] = new OpenApiString("core") } };
    }
}
