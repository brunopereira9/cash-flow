...
public sealed class KeycloakAdminClient(IHttpClientFactory clients, IConfiguration configuration) : IKeycloakAdminClient
{
    public async Task<IReadOnlyList<ManagedUser>> ListUsersAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var user in document.RootElement.EnumerateArray())
        {
            ...
            result.Add(new ManagedUser(id, user.GetProperty("username").GetString() ?? "", user.TryGetProperty("email", out var email) ? email.GetString() : null, user.GetProperty("enabled").GetBoolean(), roles.Contains("admin") ? "admin" : roles.FirstOrDefault(role => role is "operator" or "auditor") ?? ""));
        }
        return result;
    }

    public async Task<ManagedUser?> CreateUserAsync(CreateManagedUserRequest request, CancellationToken cancellationToken)
    {
        var payload = JsonSerializer.Serialize(new { username = request.Username, email = request.Email, enabled = request.Enabled, emailVerified = false });
        using var response = await SendAsync(HttpMethod.Post, "users", new StringContent(payload, Encoding.UTF8, "application/json"), cancellationToken);
        ...
        using var roleResponse = await SendAsync(HttpMethod.Post, $"users/{Uri.EscapeDataString(location)}/role-mappings/realm", new StringContent(JsonSerializer.Serialize(new[] { role }), Encoding.UTF8, "application/json"), cancellationToken);
        roleResponse.EnsureSuccessStatusCode();
        return new ManagedUser(location, request.Username, request.Email, request.Enabled, request.Role);
    }

    public async Task<bool> UpdateUserAsync(string id, UpdateManagedUserRequest request, CancellationToken cancellationToken)
    {
        if (!request.Enabled || request.Role != "admin")
        {
            ...
            var targetIsOnlyAdmin = users.Any(user => user.Id == id && user.Enabled && user.Role == "admin") && activeAdmins <= 1;
        ...
        using var response = await SendAsync(HttpMethod.Put, $"users/{Uri.EscapeDataString(id)}", new StringContent(payload, Encoding.UTF8, "application/json"), cancellationToken);
        if (!response.IsSuccessStatusCode) return false;
        var role = await RoleAsync(request.Role, cancellationToken);
        using var roleResponse = await SendAsync(HttpMethod.Post, $"users/{Uri.EscapeDataString(id)}/role-mappings/realm", new StringContent(JsonSerializer.Serialize(new[] { role }), Encoding.UTF8, "application/json"), cancellationToken);
    ...
    {
        using var response = await SendAsync(HttpMethod.Get, $"roles/{Uri.EscapeDataString(role)}", null, cancellationToken);
        response.EnsureSuccessStatusCode();
        using var document = JsonDocument.Parse(await response.Content.ReadAsStringAsync(cancellationToken));
    ...
    {
        using var response = await SendAsync(HttpMethod.Get, $"users/{Uri.EscapeDataString(id)}/role-mappings/realm", null, cancellationToken);
        response.EnsureSuccessStatusCode();
        using var document = JsonDocument.Parse(await response.Content.ReadAsStringAsync(cancellationToken));
        return document.RootElement.EnumerateArray().Select(item => item.GetProperty("name").GetString()!).ToHashSet(StringComparer.Ordinal);
    }
    ...
    private async Task<string> AdminTokenAsync(HttpClient client, string baseUri, string realm, CancellationToken cancellationToken)
    {
        var clientId = configuration["Keycloak:UserAdminClientId"] ?? throw new InvalidOperationException("Keycloak user-admin client id is required.");
        var clientSecret = configuration["Keycloak:UserAdminClientSecret"] ?? throw new InvalidOperationException("Keycloak user-admin client secret is required.");
        using var request = new HttpRequestMessage(HttpMethod.Post, $"{baseUri}/realms/{realm}/protocol/openid-connect/token")
