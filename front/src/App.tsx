...
import { createLedgerEntry, listLedgerEntries, seedLedgerEntries } from './features/ledger/api'
...
function App() {
  ...
  async function seedEntries() {
    ...
    setScreenState('loading')
    try {
      const count = await seedLedgerEntries(auth.accessToken)
      setUserMessage(`${count} lançamentos criados nos últimos 10 dias.`)
  ...
  return <main className="dashboard">
    ...
    {view === 'summary' && userMessage && <StatusBanner tone="success">{userMessage}</StatusBanner>}
    ...
    {view === 'summary' && <section className="hero">
      <div><span className="eyebrow">CONTA OPERACIONAL</span><h1>Seu dinheiro,<br /><em>em movimento.</em></h1><p>Acompanhe entradas, saídas e o saldo do seu negócio em um só lugar.</p><div className="form-actions"><button className="primary compact" onClick={() => setFormOpen(true)} disabled={screenState === 'unauthorized' || seeding}>+ Novo lançamento</button><button className="secondary compact" onClick={() => void seedEntries()} disabled={screenState === 'unauthorized' || seeding} title="Cria 2.000 lançamentos de teste, distribuídos igualmente pelos últimos 10 dias.">{seeding ? 'Gerando…' : 'Gerar carga de teste'}</button></div></div>
